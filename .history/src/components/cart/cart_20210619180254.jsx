...
const Cart = () => {  const Zero = (props) => {
    return <span className="zeroItems"> {props.zero}</span>
  }
  const { itemCount, cartItems, history } = useContext(CartContext)
  return (
    <div>
      <span className="cartItems" onClick={() => history.pushState('./CartPage')}> Cart:</span>
      {
