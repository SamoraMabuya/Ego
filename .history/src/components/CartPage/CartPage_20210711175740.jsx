...
const CartPage = () => {
  const { cartItems, itemCount, total, increase, decrease, deleteProduct} = useContext(CartContext);
  const quantityFunct = {increase, decrease, deleteProduct}
  return (
      ...
      cartItems.length === 0 ? 
       ...
       : 
        <>
          <hr className="line" />
          {cartItems.map((item) => (
            <CartItems {...item} key={item.id} {...quantityFunct} /> ))
            }
